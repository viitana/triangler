...
Shader::Shader(const char* vertex_file_path, const char* fragment_file_path)
	: vertex_file_path_(vertex_file_path), fragment_file_path_(fragment_file_path)
{
	Compile();
}
...
void Shader::Compile()
{
	// Free previous shader if recompiling
	if (id_ != -1)
	{
		glDeleteShader(id_);
	}
	...
	if (
		!LoadShaderFile(vertex_file_path_, vertex_shader_code) ||
		!LoadShaderFile(fragment_file_path_, fragment_shader_code)
		) return;
	...
	if (
		CompileShaderSource(vertex_file_path_, vertex_shader_id, vertex_shader_source_ptr) == -1 ||
		CompileShaderSource(fragment_file_path_, fragment_shader_id, fragment_shader_source_ptr) == -1
		) return;

	GLuint program_id = LinkShaders(vertex_shader_id, fragment_shader_id);

	// Free the shaders used for linking the program
	if (program_id != -1);
	{
...
const GLuint Shader::CompileShaderSource(const char* shader_path, const GLuint shader_id, const char* source_ptr)
{
